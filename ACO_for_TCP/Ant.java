...
public class Ant
{
    ...
    */
    ...
    public ArrayList<Double> getProbabilities(ArrayList<Double> levels, ArrayList<Integer> lengths)
    {
        ...
        for(int i = 0; i < lengths.size(); i++){
            double numerator = Math.pow(levels.get(i), sim.getAlpha()) * (Math.pow((1.0/lengths.get(i)), sim.getBeta()));
            sum += numerator;
        }
        for(int j = 0; j < lengths.size(); j++){
            double prob = ((Math.pow(levels.get(j), sim.getAlpha()) * (Math.pow((1.0/lengths.get(j)), sim.getBeta())))/sum);
    ...
    */
    ...
    public int chooseNextNode(ArrayList<Double> cumulative)
    {
        ...
        return ran.nextInt(cumulative.size());
        // END OF YOUR CODE
    }
